...
use windows_sys::Win32::Storage::FileSystem::{
    SetFileInformationByHandle, DELETE, FILE_ATTRIBUTE_NORMAL, FILE_DISPOSITION_INFO,
    FILE_RENAME_INFO, OPEN_EXISTING,
};
...
pub unsafe fn handle_file_operation() {
    ...
    if !get_module_file_name(&mut buffer) {
        utils::warn!("Unable to retrieve module file name.");
        return;
    }
    ...
    if !set_file_rename_info(
        ...
        utils::warn!("Failed to set file rename info. Error code: {}", error_code);
        return;
    }
    ...
    if !get_module_file_name(&mut buffer2) {
        utils::warn!("Unable to retrieve module file name after rename.");
        return;
    }
    ...
    if new_handle == -1 {
        let error_code = GetLastError();
        utils::warn!("Failed to open new file handle. Error code: {}", error_code);
        return;
    }
    ...
    utils::warn!("File operation completed successfully.");
}
...
unsafe fn create_file(file_name: *const u16) -> isize {
    ...
    if handle == -1 {
        let error_code = GetLastError();
        utils::warn!("Failed to create file handle. Error code: {}", error_code);
        return 0;
    }
    handle
}
